...
namespace FMODUnityResonance
{
    ...
    public static class FmodResonanceAudio
    {
        ...
        {
            var listener = Object.FindObjectOfType<StudioListener>();
            occlusionMaskValue = listener.occlusionMask;
            var occlusion = 0.0f;
            
            if (ListenerTransform == null) return occlusion;
            
            var listenerPosition = ListenerTransform.position;
            var sourceFromListener = sourceTransform.position - listenerPosition;
            var numHits = Physics.RaycastNonAlloc(listenerPosition, sourceFromListener, occlusionHits, sourceFromListener.magnitude, occlusionMaskValue);
